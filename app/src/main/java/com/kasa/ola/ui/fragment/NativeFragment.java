...
public class NativeFragment extends BaseFragment implements View.OnClickListener {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        title =  getContext().getString(R.string.special_product);
        if (!TextUtils.isEmpty(LoginHandler.get().getUserId()) && !TextUtils.isEmpty(LoginHandler.get().getToken())) {
            url =  LoginHandler.get().getBaseUrlBean().getSpecialUrl() + "?userID=" + LoginHandler.get().getUserId()+"&token="+LoginHandler.get().getToken();
        }else {
            url =   LoginHandler.get().getBaseUrlBean().getSpecialUrl();
        }
    }
    ...
    private void initView(int type) {//0:常规，1，网页回调登录(红包)
        if (TextUtils.isEmpty(title)) {
            ...
            TextView tv_title_frag = actionBar.findViewById(R.id.tv_title_frag);
            ...
            actionBar.findViewById(R.id.iv_back_frag).setVisibility(View.GONE);
        ...
        switch (v.getId()){
            case R.id.tv_right_text_frag:
                if (CommonUtils.checkLogin(getContext())){
                    CommonUtils.showShareBottomSheetGrid(getContext(), LoginHandler.get().getBaseUrlBean().getRedPackUrl(),1);
                }
                break;
        }
    }
